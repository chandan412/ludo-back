...
// ============================================================================
// ✅ SIGNUP + PASSWORD-RESET OTP STORE
//
...
//
// ✅ 'password_reset' ADDED TO purpose — the SAME collection and rate-limit
// machinery (daily cap, per-IP cap) now also covers forgot-password OTPs. The
// per-NUMBER resend-cooldown / hourly-send-limit lookups in utils/otp.js filter
// on { phone, purpose, consumedAt: null }, so a signup attempt and a
// password-reset attempt on the same phone number never share or fight over
// the same row — but the platform-wide daily/IP ceilings (which query without
// a purpose filter) apply across both purposes together, which is deliberate:
// it's one combined SMS-spend budget, not two separate ones an attacker could
// exhaust twice.
// ============================================================================
